const hi = 'Hej!'
const hiEng = 'Hi!'
const aboutMeEng = `Curious, social and responsible, used to treating people in a professional and genuine way regardless of context. Has been described as consulting and reliable, driven by problem solving and creative creation. A humble team player who sees it as a matter of course to both share my knowledge and bring in new ones.`

const aboutMe2Eng = 'Believe that an agile collaboration is the best way to reach new, unimaginable heights!'

const aboutMe = `Nyfiken, social och ansvarsfull, van att bemöta människor på ett professionellt och genuint sätt oavsett sammanhang. Har beskrivits som konsultmässig och pålitlig, drivs av problemlösning och kreativt skapande. En ödmjuk lagspelare som ser det som en självklarhet att både dela med mig av mina kunskaper och ta till mig nya.`
const aboutMe2 = 'Tror att ett agilt samarbete är det bästa sättet att nå nya, oanade höjder!'

const mainCompetence = 'huvudsakliga kompetenser'
const mainCompetenceEng = 'main competence'


const htmlCSS = 'HTML/CSS'
const html = 'HTML5'
const CSS = 'CSS3'
const javascript = 'JavaScript'
const react = 'ReactJS'
const typescript = 'TypeScript'
const nodejs = 'NodeJS'
const csharp = 'C# / .NET'
const entity = 'Entity'
const sql = 'SQL'
const mongodb = 'MongoDB'
const git = 'Git'
const scrum = 'Scrum'
const wordpress = 'WordPress'
const adobe = 'Adobe CC'
const swedishEng = 'Swedish '
const swedish = 'Svenska '
const english = 'Engelska'
const englishEng = 'English'

const iconText = 'Klicka här för mer information'
const iconTextEng = 'Click here for more information'
const languageText = 'Flytande i tal och skrift'
const languageTextEng = 'Fluent in writing and speech'

const linkHTML = 'https://sv.wikipedia.org/wiki/HTML5'
const linkCSS = 'https://sv.wikipedia.org/wiki/CSS3'
const linkJavascript = 'https://sv.wikipedia.org/wiki/Javascript'
const linkReact = 'https://en.wikipedia.org/wiki/React_(JavaScript_library)'
const linkTypescript = 'https://en.wikipedia.org/wiki/TypeScript'
const linkNodejs = 'https://en.wikipedia.org/wiki/Node.js'
const linkCSharp = 'https://en.wikipedia.org/wiki/C_Sharp_(programming_language)'
const linkEntity = 'https://en.wikipedia.org/wiki/Entity_Framework'
const linkSQL = 'https://en.wikipedia.org/wiki/SQL'
const linkMongodb = 'https://en.wikipedia.org/wiki/MongoDB'
const linkGit = 'https://en.wikipedia.org/wiki/Git'
const linkScrum = 'https://sv.wikipedia.org/wiki/Scrum'
const linkWordpress = 'https://en.wikipedia.org/wiki/WordPress'
const linkAdobe = 'https://en.wikipedia.org/wiki/Adobe_Creative_Cloud'


const bgcolorHTML = '#E96C4A'
const bgcolorCSS = '#039BE5'
const bgcolorJavascript = '#D6BA32'
const bgcolorReact = '#00BCD4'
const bgcolorTypescript = '#007ACC'
const bgcolorNodeJS = '#8CC84B'
const bgcolorCSharp = '#9A6CD6'
const bgcolorEntity = '#146DB2'
const bgcolorSQL = '#000000'
const bgcolorMongoDB = '#4CAF50'
const bgcolorGit = '#F05133'
const bgcolorScrum = '#424242'
const bgcolorWordpress = '#21759B'
const bgcolorAdobe = '#CD0818'

export const AboutMeDataEnglish = {
  hiEng,
  hi,
  aboutMe,
  aboutMe2,
  aboutMeEng,
  aboutMe2Eng,
  mainCompetence,
  mainCompetenceEng,
  htmlCSS,
  javascript,
  react,
  nodejs,
  csharp,
  entity,
  sql,
  mongodb,
  git,
  scrum,
  wordpress,
  adobe,
  swedish,
  swedishEng,
  english,
  englishEng,
  iconText,
  iconTextEng,
  html,
  CSS,
  languageText,
  languageTextEng,
  linkHTML,
  bgcolorHTML,
  bgcolorJavascript,
  bgcolorMongoDB,
  bgcolorNodeJS,
  bgcolorReact,
  bgcolorSQL,
  bgcolorScrum,
  bgcolorWordpress,
  linkWordpress,
  bgcolorCSS,
  bgcolorCSharp,
  bgcolorEntity,
  bgcolorGit,
  bgcolorAdobe,
  bgcolorTypescript,
  typescript,
  linkCSS,
  linkJavascript,
  linkReact,
  linkTypescript,
  linkNodejs,
  linkCSharp,
  linkEntity,
  linkSQL,
  linkMongodb,
  linkGit,
  linkScrum,
  linkAdobe,
}